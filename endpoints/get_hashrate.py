...
@app.get("/info/hashrate", response_model=HashrateResponse | str, tags=["Kobra network info"])
async def get_hashrate(stringOnly: bool = False):
    """
    Returns the current hashrate for Kobra network in TH/s.
    """

    resp = await kobrad_client.request("getBlockDagInfoRequest")
...
@app.get("/info/hashrate/max", response_model=MaxHashrateResponse, tags=["Kobra network info"])
@sql_db_only
