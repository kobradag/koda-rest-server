...
@app.get("/addresses/{kobraAddress}/transactions",
         response_model=TransactionForAddressResponse,
         response_model_exclude_unset=True,
         tags=["kobra addresses"],
         deprecated=True)
@sql_db_only
async def get_transactions_for_address(
        kobraAddress: str = Path(
            description="Kobra address as string e.g. "
                        "kobra:qrvwazapajffhp5mmc65uwh8d887yckdhheu3hayqrzhf5mt3h7cxad0zuyzh",
    ...
    #
    # WHERE "script_public_key_address" = 'kobra:qrvwazapajffhp5mmc65uwh8d887yckdhheu3hayqrzhf5mt3h7cxad0zuyzh'
    #
    ...
    async with async_session() as session:
        resp = await session.execute(text(f"""
            ...
            WHERE "script_public_key_address" = :kobraAddress
			ORDER by transactions.block_time DESC
			LIMIT 500"""),
                                     {'kobraAddress': kobraAddress})
...
@app.get("/addresses/{kobraAddress}/full-transactions",
...
async def get_full_transactions_for_address(
    ...
    async with async_session() as s:
        ...
        tx_within_limit_offset = await s.execute(select(TxAddrMapping.transaction_id)
                                                 .filter(TxAddrMapping.address == kobraAddress)
                                                 .limit(limit)
                                                 .offset(offset)
...
@app.get("/addresses/{kobraAddress}/transactions-count",
...
async def get_transaction_count_for_address(
    ...
    async with async_session() as s:
        count_query = select(func.count()).filter(TxAddrMapping.address == kobraAddress)
